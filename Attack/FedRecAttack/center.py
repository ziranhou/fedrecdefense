...
from DL_FRS.parse import args

class FedRecAttackCenter(nn.Module):
    def __init__(self, users_train_ind, target_items, m_item, dim):
        super().__init__()
        # center.py 中的 __init__ 方法里修改
        # 确保每个用户的物品是一个列表而不是一个单一的数值
        self.users_train_ind = {user: [item] for user, item in enumerate(users_train_ind)}
    ...
    def loss(self, batch_users):
        ...
        for user in batch_users:
            user = int(user)  # 强制转为int，避免 float64 报错
    ...
    def train_(self, new_items_emb, batch_size, eps=1e-4):
        ...
        if len(self.users_train_ind):
            rand_users = np.arange(self.n_user, dtype=int)  # 确保是整数类型
